...
define(['N/search', 'N/runtime', 'N/record', './rsc_junix_call_api.js'],
    ...
    (search, runtime, record, api) => {
        ...
        const getInputData = (inputContext) => {
                return search.create({
                        type: "customrecord_rsc_bl_emp",
                         filters:
                             [
                                     ["custrecord_rsc_atualziado_bloco_junix","is","F"],
                             ],
                        columns:
                            [
                                    search.createColumn({
                                    ...
                                    search.createColumn({name: "custrecord_rsc_id_bloco_emp_junix", label: "ID Bloco"}),
                                    search.createColumn({name: "custrecord_rsc_bl_emp_unid_andar", label: "Unidade Andar"}),
        ...
        const map = (mapContext) => {
                ...
                log.debug({title: 'Cod Sub', details: projeto.getValue('entityid')});
                ...
                var codProject = projeto.getValue('custentity_rsc_id_empreendimento_junix')
                // log.debug({title: 'entityid[0]', details: entityid[0]});
                /* montar o body do request */
                var body = {
                        codigo: searchResult.values['custrecord_rsc_id_bloco_emp_junix'],
                        ...
                        quantidadeUnidades: searchResult.values['custrecord_rsc_bl_emp_unid_andar'],
                        dataAgi: 0
                }
                ...
                if (retorno.OK){
                        ...
                        job_update.setValue('custrecord_rsc_atualziado_bloco_junix', true);
                        job_update.setValue('custrecord_rsc_bl_emp_idJunix', retorno.Dados);
                        job_update.setValue('custrecord_rsc_status_bloco_junix', '');
                        
                        
                } else {
                        job_update.setValue('custrecord_rsc_status_bloco_junix', 'Preencha o campo ' + retorno.CampoErros[0].Campo);
