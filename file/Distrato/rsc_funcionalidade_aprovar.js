...
define(["require", "exports", "N/record", "N/search", "N/redirect", "N/task", "N/runtime", "N/https", "N/log"], function (require, exports, record_1, search_1, redirect_1, task_1, runtime_1, https_1, log_1) {
    ...
    var onRequest = function (ctx) {
        ...
        var distratoRecord = record_1.default.load({
            type: 'customrecord_rsc_escritura_distrato_2',
            id: params.distratoId
        });
        var qtdParcelas = distratoRecord.getValue('custrecord_rsc_qtd_parcelas_2');
        var dataVencimento = distratoRecord.getValue('custrecord_rsc_vencimento_inicial_2');
        log_1.default.error("data", new Date(dataVencimento));
        dataVencimento = new Date(dataVencimento);
        var valorAcordado = distratoRecord.getValue('custrecord_rsc_valor_acordado_2');
        var trasnWorkflow = 8;
        ...
        //     type: 'customer',
        //     id: distratoRecord.getValue('custrecord_rsc_cliente_distrato_2'),
        ...
        // }
        // else {
        //     idFornecedor = getIdnewFornecedor(distratoRecord.getValue('custrecord_rsc_cliente_distrato_2'));
        // }
        var loc = buscaLocalidade(distratoRecord.getValue('custrecord_rsc_cliente_distrato_2'));
        for (var i = 0; i < Number(qtdParcelas); i++) {
            ...
            newVendorbill.setValue({
                fieldId: 'entity',
                value: distratoRecord.getValue('custrecord_rsc_cliente_distrato_2')
            });
            newVendorbill.setValue({
                fieldId: 'location',
                value: loc
            ...
            newVendorbill.setValue({
                ...
                value: distratoRecord.getValue('custrecord_rsc_empreedimento_distrato_2')
            });
            newVendorbill.setValue({
        ...
        var unidade = distratoRecord.getValue('custrecord_rsc_unidade_distrato_2');
        var recordUnidade = record_1.default.load({
            id: unidade,
            type: 'customrecord_rsc_unidades_empreendi_2'
        });
        recordUnidade.setValue({
            fieldId: 'custrecord_rsc_un_emp_status_2',
            value: 1
        });
        recordUnidade.setValue({
            fieldId: 'custrecord_rsc_un_emp_nr_contrato_2',
            value: ""
        });
        recordUnidade.save({
            ignoreMandatoryFields: true
        ...
        distratoRecord.setValue({
            fieldId: 'custrecord_rsc_status_distrato_2',
            value: 5
        });
        distratoRecord.setValue({
            fieldId: 'custrecord_rsc_cnt_pagar_2',
            value: idVendorbill
        });
        distratoRecord.setValue({
            fieldId: 'custrecord_rsc_status_cancelamento_2',
            value: 'Em andamento'
        });
        distratoRecord.save({
            ignoreMandatoryFields: true
    ...
    var getAttMonetaria = function (dataVencimento, indiceID, valorPago) {
        ...
        if (new Date("01/01/2022").getTime() > dataVencimento.getTime()) {
            ...
            var indiceRecord = record_1.default.load({
                type: 'customrecord_rsc_correction_unit_2',
                id: indiceID
            });
            ...
            for (var i = 0; i < qtdLinhasIndice; i++) {
                ...
                var dataVigenciaLinha = indiceRecord.getSublistValue({
                    sublistId: 'recmachcustrecord_rsc_hif_correction_unit',
                    fieldId: 'custrecord_rsc_hif_effective_date_2',
                    line: i
                });
                ...
                if (String(dataVigenciaLinha) == String(dataVencimento)) {
                    ...
                    fatorDatavencimento = indiceRecord.getSublistValue({
                        sublistId: 'recmachcustrecord_rsc_hif_correction_unit',
                        fieldId: 'custrecord_rsc_hif_factor_percent_2',
                        line: i
                    });
