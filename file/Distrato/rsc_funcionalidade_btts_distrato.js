...
define(["require", "exports", "N/search", "N/record", "N/currentRecord", "N/runtime", "N/url", "N/log", "N/ui/dialog"], function (require, exports, search_1, record_1, currentRecord_1, runtime_1, url_1, log_1, dialog_1) {
    ...
    var aprovarLanc = function () {
        ...
        console.log('somatório', {
            ...
            attMonetariaAcumulada: attMonetariaAcumulada,
            valorProRata: valorProRata,
            valorPago: valorPago
        });
    ...
    var getProRataParcela = function (dataVencimento, indiceID, valorPago) {
        console.log('getProRataParcela', {dataVencimento: dataVencimento, indiceID: indiceID, valorPago: valorPago});
        var valor = 0;
        ...
        var dv = {
            dia: dataVencimento.getDate() > 9 ? dataVencimento.getDate() : '0'+dataVencimento.getDate(),
            mes: dataVencimento.getMonth() > 9 ? dataVencimento.getMonth()+1 : '0'+(dataVencimento.getMonth()+1),
            ano: dataVencimento.getFullYear()
        }
        var dvStr = String(dv.dia + '/' + dv.mes + '/' + dv.ano);        
        var dv_historicoFator = indiceRecord.findSublistLineWithValue({
        ...
        dataHoje.setDate(1);
        dataHoje.setMonth(dataHoje.getMonth() - 2);
        dataHoje.setHours(0, 0, 0, 0);
        var dh = {
            dia: dataHoje.getDate() > 9 ? dataHoje.getDate() : '0'+dataHoje.getDate(),
            mes: dataHoje.getMonth() > 9 ? dataHoje.getMonth()+1 : '0'+(dataHoje.getMonth()+1),
            ano: dataHoje.getFullYear()
        }
        var dhStr = String(dh.dia + '/' + dh.mes + '/' + dh.ano);        
        var dh_historicoFator = indiceRecord.findSublistLineWithValue({
            sublistId: 'recmachcustrecord_rsc_hif_correction_unit',
            fieldId: 'custrecord_rsc_hif_effective_date',
            value: dhStr
        });
        ...
        //     dv: {dv: dv, dvStr: dvStr, dv_historicoFator: dv_historicoFator, fatorDatavencimento: fatorDatavencimento},
        //     dh: {dh: dh, dhStr: dhStr, dh_historicoFator: dh_historicoFator, fatorDataAtual: fatorDataAtual}
        // }));
        if (fatorDatavencimento > 0) {
            valor = (Number(fatorDataAtual) / Number(fatorDatavencimento) - 1) * valorPago;
            valor = Number(valor / 30 * new Date().getDate());
            console.log({fatorDataAtual: fatorDataAtual, fatorDatavencimento: fatorDatavencimento, valor: valor, valorPago: valorPago});
            return valor; 
        } else {
            return valor;
        }
    ...
    var getProRata = function (dataVencimento, idPrincipal) {
        console.log('getProRata', {dataVencimento: dataVencimento, idPrincipal: idPrincipal})
        var soma = 0
        var repeteArray = []
        search_1.default.create({
            ...
            if(repeteArray.indexOf(repeteArray) == -1){
                ...
                // console.log('search_1.default', {invoiceRecord: invoiceRecord.id, itemDisplay: itemDisplay, amount: amount, soma: soma});
