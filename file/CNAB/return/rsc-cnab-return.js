            ...
            try {
                if (Number(occurrence.status) == 4)
                {
                    ...
                    if (queryResult.length > 0) {
                        for (var prop in queryResult) {
                            if (queryResult.hasOwnProperty(prop) && (queryResult[prop].custrecord_rsc_cnab_transpay != null && queryResult[prop].custrecord_rsc_cnab_transpay != '')) {                                
                                var paymentTrans = queryResult[prop].custrecord_rsc_cnab_transpay
                                log.error(prop, {paymentTrans: paymentTrans});       
                                
                                // TRATATIVA PARA REVERTER A ETAPA DA TRANSAÇÃO NA FATURA DO FORNECEDOR.
                                var load_payment_trans = record.load({type: 'vendorpayment', id: paymentTrans});
                                log.error('load_payment_trans', load_payment_trans);

                                var campos = {};

                                var linhasAplicadas = load_payment_trans.getLineCount('apply');
    
                                for (i=0; i<load_payment_trans.getLineCount('apply'); i++) {
                                    log.error(i, linhasAplicadas);

                                    var objAplicado = {
                                        apply: load_payment_trans.getSublistValue('apply', 'apply', i),
                                        internalid: load_payment_trans.getSublistValue('apply', 'internalid', i),
                                        trantype: load_payment_trans.getSublistValue('apply', 'trantype', i),
                                        type: load_payment_trans.getSublistValue('apply', 'type', i)
                                    }                        
                                    log.error('objAplicado', objAplicado);
    
                                    if (objAplicado.trantype == 'VendBill' && objAplicado.apply == true) {
                                ...
                                log.error(prop, 'Revertendo o pagamento: '+paymentTrans);
