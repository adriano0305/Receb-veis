 ...
 */
define(['N/file', 'N/record', 'N/query', 'N/runtime', './rsc_module_aprop_querys', '../Junix/rsc_finan_createproposta.js', 'N/task','N/url', 'N/email', 'N/search'],
    
    (file, record, query, runtime , apropQuery,proposta, task, url, email, search) => {
        ...
        function _getRateioContaAndValue(componenteId, valor, period, subsidiary, job){
                // log.audit('_getRateioContaAndValue', {componenteId: componenteId, period: period, job: job});
                ...
                //     log.audit('_getRateioContaAndValue', {results: results});
                    if (results.length > 0){
                            var response = [];
                            /* Calcula Valor total. */
                            var valorTotal = 0;
        ...
        function getSPEObra(codEmpreend, idProjeto) {
                // log.audit('getSPEObra', {codEmpreend: codEmpreend, idProjeto: idProjeto});
                ...
                //     log.debug('records', records);
                    if (records.length > 0) {
                        ...
                        //     log.debug({title: 'CodEmpreendimento', details: codEmpreendimento});
                            ...
                            // Pesquisa o empreendimento pela subsidiária.
                            var searchJob = search.create({
                                    type: 'job',
                                    filters: [
                                            ['subsidiary', 'IS', codEmpreendimento],
                                            "AND", 
                                            ["internalid","anyof",idProjeto]
                                    ]
                            }).run().getRange({
                                    start: 0,
                                    end: 1
                            })
                        //     log.audit('searchJob', searchJob);
                            // Pega o primeiro projeto da lista.
                            if(searchJob[0]){
        ...
        */
                ...
                //     log.audit('keyNames', keyNames);
                    ...
                    for (var i = 0; i < keyNames.length; i++) {
                            if (i == 0){
                                ...
                                //     log.audit('pesqSub', pesqSub);
                                    var sub = getSPEObra(pesqSub, record.id).codEmpreendimento;
                                        ...
                                        //     log.debug({title: 'Contas i > 1', details: contas});
                                            ...
                                            if (valor !=  0){
                                                    ...
                                                    if (continuar){
                                                            ...
                                                            if (rateio === 't'){
                                                                //     log.error({title:'Contas rateio === "t"', details: mapContext})
                                                                    var contas = _getRateioContaAndValue(componenteId, valor, period, sub,job);
                                                                //     log.error({title:'Contas _getRateioContaAndValue', details: contas})
                                                                    var typelancamento1 = '';
                                                                    var typelancamento2 = '';
                                    ...
                                    if (context[0].naoLancarDiferenca === 't'){
                                        ...
                                        if (accountBalance < 0){
                                                ...
                                                if (context[0].contaCredora === 't'){
                                                        ...
                                                        log.audit({contaCredora: context[0].contaCredora === 't'}, accountBalance);
                                                }
                                        }
                                    }
        ...
        const summarize = (summaryContext) => {
            log.audit('summarize', summaryContext);
