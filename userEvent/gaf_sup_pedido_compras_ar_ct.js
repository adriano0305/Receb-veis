...
define(['N/currentRecord', 'N/log', 'N/runtime', 'N/ui/dialog', 'N/search'], (currentRecord, log, runtime, dialog, search) => {
...
const pageInit = (context) => {
    ...
    if (registroAtual.type == 'purchaseorder') {
        ...
        var etapaProjeto = registroAtual.getValue('class');

        if (etapaProjeto && (etapaProjeto != null || etapaProjeto != '')) {
            campos.class = registroAtual.getField({fieldId: 'class'});
        }
    ...
    if (corpo.departamento || corpo.nomeProjeto) {
        ...
        if (linhas.itens > 0 || linhas.despesas > 0) {
            for (i=0; i<registroAtual.getLineCount({sublistId: sublista}); i++) {
                ...
                var transacaoWorkflow = registroAtual.getValue('custbody_rsc_tipo_transacao_workflow')
                ...
                if (departamentoLinha) { camposPreenchidos.push(departamentoLinha); }
                if (clienteLinha) { camposPreenchidos.push(clienteLinha); }

                if (transacaoWorkflow != 21) {
                    if (corpo.departamento && !departamentoLinha) {
                        dialog.alert({
                            title: 'ERRO!',
                            message: '- Caso o campo: "DEPARTAMENTO" esteja prenchido, o Item/Despesa deve possuir o campo "DEPARTAMENTO" preenchido. <br> <br>' + '- Caso o campo: "NOME DO PROJETO" esteja prenchido, o Item/Despesa deve possuir o campo "NOME DO PROJETO" preenchido.' 
                        });
                        return false 
                    }
    
                    if (corpo.nomeProjeto && !clienteLinha) {
                        dialog.alert({
                            title: 'ERRO!',
                            message: '- Caso o campo: "DEPARTAMENTO" esteja prenchido, o Item/Despesa deve possuir o campo "DEPARTAMENTO" preenchido. <br> <br>' + '- Caso o campo: "NOME DO PROJETO" esteja prenchido, o Item/Despesa deve possuir o campo "NOME DO PROJETO" preenchido.' 
                        });
...
const fieldChanged = (context) => {
    ...
    if (sublista != null && (sublista == 'expense' || sublista == 'item')) {    
        ...
        if (campo == 'customer') {
            registroAtual.selectLine({sublistId: sublista, line: linha})
            valor = registroAtual.getCurrentSublistValue({sublistId: sublista, fieldId: campo});   
            valorTexto = registroAtual.getCurrentSublistText({sublistId: sublista, fieldId: campo});   
            if (valor && valorTexto) {
                var buscaTarefa = search.create({
                    type: 'job',
                    filters: [
                        ["internalId", "IS", valor],
                        "AND",
                        ["entityid", "IS", valorTexto]
                    ],
                    columns: ['internalId'],
                }).run().getRange(0,1)[0]
                travaSave = 'Entrei'
                confirmClientTaref = buscaTarefa
                if (!buscaTarefa) {
                    dialog.alert({
                        title: 'ERRO!',
                        message: 'O campo: NOME DO PROJETO (CLIENTE: TAREFA) está preenchido com um registro que não é um projeto!'
                    })
                    registroAtual.setCurrentSublistValue({sublistId: sublista, fieldId: campo, value: ''});
                }
            }
...
const validateLine = (context) => {
    ...
    if (sublista != 'mediaitem' && (sublista == 'expense' || sublista == 'item')) {
