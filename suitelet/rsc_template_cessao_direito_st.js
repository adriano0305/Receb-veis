...
define(['N/config', 'N/email', 'N/file', 'N/format', 'N/log', 'N/query', 'N/record', 'N/render', 'N/runtime', 'N/search', 'N/xml'], (config, email, file, format, log, query, record, render, runtime, search, xml) => {
const atualizaCessaoDireito = (dados) => {
    // log.audit('atualizaCessaoDireito', dados);
    ...
    .save();
    log.audit('Sucesso', {dados: dados});
}

const funcionario = (idInterno) => {
    var lookupFunc = search.lookupFields({type: 'employee',
        id: idInterno,
        columns: ['email']
    });
    log.audit('lookupFunc', lookupFunc);

    return lookupFunc.email;
}
...
const cliente = (entitytitle, doc) => {
    var sql = "SELECT custentity_enl_cnpjcpf, custentity_lrc_rg, email "+
    "FROM customer "+
    "WHERE entitytitle = ? ";
    ...
    // return doc == 'cpf' ? 
    // ((sqlResults[0] && sqlResults[0].custentity_enl_cnpjcpf != null) ? sqlResults[0].custentity_enl_cnpjcpf : 'Não cadastrado') : 
    // ((sqlResults[0] && sqlResults[0].custentity_lrc_rg != null) ? sqlResults[0].custentity_lrc_rg : 'Não cadastrado');  

    if (doc == 'cpf') {
        return ((sqlResults[0] && sqlResults[0].custentity_enl_cnpjcpf != null) ? sqlResults[0].custentity_enl_cnpjcpf : 'Não cadastrado');
    } else if (doc == 'rg') {
        return ((sqlResults[0] && sqlResults[0].custentity_lrc_rg != null) ? sqlResults[0].custentity_lrc_rg : 'Não cadastrado');
    } else {
        return sqlResults[0].email;
    }
}
...
const logo = () => {
    var bscImagem = search.create({type: "file",
        filters: [
            ["internalid","anyof","11905"]
        ],
        columns: [
            "name","folder","documentsize","url","created","modified","filetype"
        ]
    }).run().getRange(0,1);
    log.audit('bscImagem', bscImagem);

    if (bscImagem.length > 0) {
        var fileObj = file.load({id: bscImagem[0].id});
        var urlQA = "https://5843489-sb2.app.netsuite.com";
        var imgURLForPDF = fileObj.url;
        var newURL = (urlQA + imgURLForPDF).replace(/&/g, '&amp;');
        log.audit('newURL', newURL);
        return newURL;
    }
}
...
const onRequest = (context) => {
    ...
    const img_tag = "https://5843489-sb2.app.netsuite.com/core/media/media.nl?id=11905&c=5843489_SB2&h=_n2RlM5jDc42Nq7NKrXFFgm6LI4tGNv71DlvUMmitWAhZG3z";
    log.audit('img_tag', img_tag);
    ...
    if (metodo == 'POST') {
        try {
            ...
            template += '<td><img src="' + logo() + '" height="20pt" width="100" align="center"/></td>';
            template += '</tr></table>';
            template += '</macro>';
            ...
            template += '</style>'; 
            template += '</head>';
            template += '<body header="nlheader" header-height="5%" footer="nlfooter" footer-height="20pt" padding="0.5in 0.5in 0.5in 0.5in" size="Letter" font-size="8">';
            ...
            body.proponentes.forEach(function (bidder) {
                ...
                template += '<td>';
                template += cliente(body.clienteAtual.text, 'cpf');
                template += '</td>';

                template += '<td>';
                template += body.clienteAtual.text;
                template += '</td>';
                ...
                template += '<td>';
                template += cliente(body.clienteAtual.text, 'rg');
                template += '</td>';
            ...
            body.novosProponentes.forEach(function (newBidder) {
                ...
                template += '<td>';
                template += newBidder.rg ? newBidder.rg : 'Não cadastrado';
                template += '</td>';
            ...
            template += '<td><b>Feito por: </b>'+body.criadorCD.text+'</td>';
            ...
            var dadosEmail = {
                destinatario: funcionario(body.criadorCD.value),
                copia: [cliente(body.clienteAtual.text, 'email'), cliente(body.novoCliente.text, 'email')]
            }
            log.audit('dadosEmail', dadosEmail);
            ...
            if (!body.ordem) {
                email.send({
                    ...
                    // cc: copia,
                    recipients: dadosEmail.destinatario,
                    cc: dadosEmail.copia, 
            ...
            atualizaCessaoDireito({
                id: body.recordId, 
                custrecord_rsc_erro_perc_cessao_direito: JSON.stringify(erro)
            });
        }    

        if (!body.ordem) {
            response.write(idcessao ? JSON.stringify({status: 'Sucesso'}) : JSON.stringify({status: 'Erro', msg: 'Arquivo pdf não gerado!'}));
        } else if (body.ordem) {
            response.write(idcessao ? JSON.stringify({status: 'Sucesso', pdf: file.load(idcessao).url}) : JSON.stringify({status: 'Erro', msg: 'Arquivo pdf não gerado!'}));   
        } else if (erro) {
            response.write(idcessao ? JSON.stringify({status: 'Erro', pdf: file.load(idcessao).url, msg: erro}) : JSON.stringify({status: 'Erro', msg: erro})); 
        }
    }
}
