...
const atualizarReneg = (idReneg, campos) => {
    log.audit('atualizarReneg', {idReneg: idReneg, campos: campos});
    ...
    try {
        Object.keys(campos).forEach(function(bodyField) {
            loadReg.setValue(bodyField, campos[bodyField]);
        });
        ...
        record.submitFields({type: 'customrecord_rsc_tab_efetiva_reparcela',
            id: idReneg,
            values: {
                custrecord_rsc_erro_renegociacao: e
            },
            options: opcoes            
        });
...
const gerarBoleto = (dados) => {
    ...
    const unidadeCorrecao = (idUC) => {
        ...
        if (lkpUC.custrecord_rsc_ucr_calc_base_item[0]) {
            return lkpUC.custrecord_rsc_ucr_calc_base_item[0].value;
        } else {
            valores.custrecord_rsc_erro_renegociacao = 'INDICE: "' + lkpUC.name + '" - "Item para base de cálculo de adição e subtração" não preenchido.';
            atualizarTE = atualizarReneg(dados.idRenegociacao, valores);
            return lkpUC.custrecord_rsc_ucr_calc_base_item[0].value;
        }
    ...
    try {
        ...
        valores.custrecord_rsc_boleto = id_memo_credito;
        valores.custrecord_rsc_status_aprovacao = 2; // Aprovado
        valores.custrecord_rsc_erro_renegociacao = "";
        ...
        valores.custrecord_rsc_erro_renegociacao = e;
        // TE (Tabela de Efetivação)
        atualizarTE = atualizarReneg(dados.idRenegociacao, valores);
        return {
            status: 'Erro', 
