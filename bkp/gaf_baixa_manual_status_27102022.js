    ...
    (log, record) => {
        ...
        const baixaManual = (newRecord) => {
            ...
            for(var i = 0; i<lines; i++){
                ...
                var internalid = newRecord.getSublistValue({
                    sublistId:'apply',
                    fieldId: 'internalid',
                    line: i
                });
                var nInstallment = newRecord.getSublistValue({
                    sublistId:'apply',
                    fieldId: 'installmentnumber',
                    line: i
                });
                var trantype =  newRecord.getSublistValue({
                    sublistId:'apply',
                    fieldId: 'trantype',
                    line: i
                });
                ...
                if(trantype == 'VendBill'){
                    if(apply){
                        ...
                        log.debug('dataPago', dataPago)
                        var fatura = record.load({
                            type:'vendorbill',
                            id: internalid
                        })
                        var linhasFatura = fatura.getLineCount({
                            sublistId:'installment'
                        })
                        log.debug('linhasFatura', linhasFatura)
                        for(var j = 0; j < linhasFatura;j++){
                            var seqNum = fatura.getSublistValue({
                                sublistId:'installment',
                                fieldId: 'seqnum',
                                line: j
                            });
                            var statusCnab = fatura.getSublistValue({
                                sublistId:'installment',
                                fieldId: 'custrecord_rsc_cnab_inst_status_ls',
                                line: j
                            });
                            var statusInstallment = fatura.getSublistValue({
                                sublistId:'installment',
                                fieldId: 'status',
                                line: j
                            });
                            log.debug('seqNum', seqNum)
                            log.debug('nInstallment', nInstallment)
                            log.debug('statusCnab', statusCnab)
                            if(seqNum == nInstallment){
                                if(statusCnab == 1){
                                    fatura.setSublistValue({
                                        sublistId:'installment',
                                        fieldId: 'custrecord_rsc_cnab_inst_status_ls',
                                        line: j,
                                        value: 6
                                    ...
                                    fatura.setSublistValue({
                                        sublistId:'installment',
                                        fieldId: 'custrecord_rsc_cnab_inst_paymentdate_dt',
                                        line: j,
                                        value: dataPago
                                    });
                                    statusCnab = 6
