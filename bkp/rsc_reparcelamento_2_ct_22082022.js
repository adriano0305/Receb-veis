...
define(['./gaf_selecao_parcelas_ar_lib.js', 'N/currentRecord', 'N/log', 'N/record', 'N/runtime', 'N/search', 'N/ui/dialog', 'N/url'], function(gspal, currentRecord, log, record, runtime, search, dialog, url) {
function juros_e_acrescimos_moratorios(id) {
    console.log('juros_e_acrescimos_moratorios', id);

    var loadReg = record.load({type: 'invoice', id: id});

    // Acréscimos moratórios
    var linha_acrescimos_moratorios = loadReg.findSublistLineWithValue('item', 'item', 30694); 
    console.log('linha_acrescimos_moratorios', linha_acrescimos_moratorios);

    var acrescimosMoratorios;
    if (linha_acrescimos_moratorios != -1) {
        acrescimosMoratorios = loadReg.getSublistValue('item', 'amount', linha_acrescimos_moratorios);
    } else {
        acrescimosMoratorios = 0;
    }    
    console.log('acrescimosMoratorios', acrescimosMoratorios);
...
function recalcJuros(novoVencimento, job, arrayParcelas, am) {
    ...
    const jurosAA = job.jurosEmpreendimento ? (job.jurosEmpreendimento / 100) : ''; // a.a
    const multa = job.multaEmpreendimento ? (job.multaEmpreendimento / 100) : '';   
    ...
    arrayParcelas.forEach(function(parcela) {
        ...
        var valor_atualizacaoMonetaria, juros, valorAtualizado, mj;
...
function simulacao(ordem) {
    ...
    const objJob = {
        ...
        jurosEmpreendimento: registroAtual.getValue({fieldId: custPage+'juros_empreendimento'}),
        multaEmpreendimento: registroAtual.getValue({fieldId: custPage+'multa_empreendimento'})
    }
    ...
    for (i=0; i<registroAtual.getLineCount({sublistId: listaParcelas}); i++) {
        ...
        if (reparcelar == true) {
            arrayParcelas.push({
                ...
                ultimaAtualizacao: registroAtual.getSublistText({
                    sublistId: listaParcelas,
                ...
                fator_atual: registroAtual.getSublistValue({
                    sublistId: listaParcelas,
                    fieldId: custPage+'fator_atual',
                    line: i
                }),
                moratorios: registroAtual.getSublistValue({
                    sublistId: listaParcelas,
                    fieldId: custPage+'moratorios',
                    line: i
                })
            });
    ...
    if (renegociacao == 'Amortização') {
        ...
        ultimaAtualizacao = arrayParcelas[0].ultimaAtualizacao.split('/');
        // console.log({ultimaAtualizacao: ultimaAtualizacao, typeof: typeof(ultimaAtualizacao)});
        ...
        calculoPR = calcularProRata(renegociacao, new Date(registroAtual.getValue({fieldId: custPage+'data_inicio'})), arrayParcelas, registroAtual, new Date(), calculoAM);    
        ...
        json.campanhaDesconto = campanhaDesconto(primeiroVencimento, json.somatorioParcelas.valorAtualizado); 
    ...
    if (renegociacao == 'Inadimplentes' || renegociacao == 'Adimplentes' || renegociacao == 'Recálculo de atrasos' || renegociacao == 'Antecipação') {
        ...
        for (i=0; i<arrayParcelas.length; i++) {
            ...
            // ultimaAtualizacao = lkpFI[0].custbody_rsc_ultima_atualizacao ? lkpFI[0].custbody_rsc_ultima_atualizacao.split('/') : '';
        ...
        calculoPR = calcularProRata2(renegociacao, new Date(registroAtual.getValue({fieldId: custPage+'vencimento_entrada'})), arrayParcelas, registroAtual, new Date(), calculoAM);
        ...
        if (renegociacao == 'Recálculo de atrasos' || renegociacao == 'Antecipação' || renegociacao == 'Inadimplentes') {
            ...
            if (renegociacao == 'Antecipação') {
                json.campanhaDesconto = campanhaDesconto(vencimentoEntrada, json.somatorioParcelas.valorAtualizado); 
            }             
        }
...
function campanhaDesconto(data, valor) {
    ...
    var nesteMes = String(dia+'/'+mes+'/'+ano);

    var vigenciaInicio, vigenciaFim;

    var split = nesteMes.split('/');    
...
function calcularProRata2(reneg, dt1, dt2, registroAtual, dt3, am) {    
    ...
    dt2.forEach(function(dt) {
        ...
        var fator_anterior2 = dt.fator_correcao_2;
        ...
        var fator_atual = dt.fator_atual;
...
function atualizacaoMonetaria(reneg, dt1, dt2) {
    ...
    var valor_atualizacao_monetaria = 0;

    if (reneg == 'Amortização') {
        fator_anterior2 = dt2[0].fator_correcao_2;
    ...
    console.log('valor_atualizacao_monetaria '+reneg, valor_atualizacao_monetaria); 
...
function calcularProRata(reneg, dt1, dt2, registroAtual, dt3, am) {    
    ...
    // const format_dt2 = new Date(splitDt2[2], splitDt2[1], splitDt2[0]);
    const format_dt2 = new Date(splitDt2[2], splitDt2[1] - 1, splitDt2[0]);
    // console.log('format_dt2', format_dt2);
    ...
    const fator_anterior3 = dt2[0].fator_correcao_3;
    // console.log('fator_anterior3: '+fator_anterior3, 'typeof: '+typeof(fator_anterior3));
    ...
    const fator_atual = dt2[0].fator_atual || 0;
    // console.log('fator_atual', fator_atual);
...
function fatorCorrecao(mes, status, indice) {
    ...
    if (status == 'anterior2') {
        ...
        // console.log(JSON.stringify({strPD: strPD, strUD: strUD}));
        ...
        if (bsc_UnidadeCorrecao.length > 0) {
            console.log('anterior', Number(bsc_UnidadeCorrecao[0].custrecord_rsc_hif_factor_percent).toFixed(6) || 0);
