        ...
        case 'invoice':                
            ...
            if (tipo == 'edit') {                    
                ...
                if (anulado == 'T' || anulado == true) {
                    ...
                    if (tipoRenegociacao == 5 && (!pago || pago == 2)) { // Antecipação
                        bscTransacao = search.create({type: "journalentry",
                            filters: [
                                ["shipping","is","F"], "AND", 
                                ["taxline","is","F"], "AND", 
                    ...
                    for (i=0; i<novoRegistro.getLineCount('apply'); i++) {
                        ...
                        if (aplicado == true && tipoTransacao == 'CustInvc') {
                            ...
                            for (j=0; j<loadReg.getLineCount('item'); j++) {
                                ...
                                if (item == 28653) { // JUROS PRICE INCORRIDO
                                    conta = search.lookupFields({type: 'serviceitem',
                                        id: parametroScript.getParameter('custscript_rsc_cdt_juros_incorridos_1'),
                                        columns: ['incomeaccount']
                                    }).incomeaccount[0].value;
                                ...
                                if (item == 28654 || item == 30694) { // JUROS PRICE A INCORRER OU ACRÉSCIMO SOBRE FINANCIAMENTO 
                                    conta = search.lookupFields({type: 'serviceitem',
                                        id: parametroScript.getParameter('custscript_rsc_cdt_juros_incorrer_1'),
                                        columns: ['incomeaccount']
                                    }).incomeaccount[0].value;
            ...
            if (tipo == 'create') {
                for (i=0; i<novoRegistro.getLineCount('apply'); i++) {
                    ...
                    if (aplicado == true && tipoTransacao == 'CustInvc') {
                        ...
                        var nomeProjeto = {
                            text: loadReg.getText('custbody_rsc_projeto_obra_gasto_compra'),
                            value: loadReg.getValue('custbody_rsc_projeto_obra_gasto_compra')
                        }
                        ...
                        var lkpProjeto = search.lookupFields({type: 'job',
                            ...
                            columns: [
                                'companyname','custentity_rsc_securitizado','custentity_rsc_d_baixa_clientes','custentity_rsc_c_baixa_clientes','custentity_rsc_d_receb_clientes','custentity_rsc_c_receb_clientes'
                            ]
                        });
