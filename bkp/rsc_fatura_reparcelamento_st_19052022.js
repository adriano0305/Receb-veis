    ...
    * *************************************************** */ 
    ...
    if (method == 'GET') {
        ...
        var tipoParcela = sublistaReparcelamento.addField({
            ...
            type: serverWidget.FieldType.SELECT,
            label: 'Tipo Parcela',
            source: 'customlist_rsc_tipo_parcela'
        });
...
function localizarParcelas(idFatura) {
    ...
    't.custbody_rsc_reparcelamento_destino, t.foreigntotal, t.foreignamountpaid, t.foreignamountunpaid, t.custbody_rsc_amortizada, t.custbody_rsc_indice, t.status, '+
    ...
    if (sqlResults.length > 0) {        
        const lookupEmpreendimento = search.lookupFields({type: 'job',
        ...
        const jurosAA = lookupEmpreendimento.custentity_rsc_juros.replace('%','') / 100; // a.a
        const multa = lookupEmpreendimento.custentity_rsc_multa.replace('%','') / 100;       
        ...
        for (var prop in sqlResults) {
            if (sqlResults[prop].item != null || sqlResults[prop].item != 5) {
                if (sqlResults[prop].status === 'A' || sqlResults[prop].status === 'B' && sqlResults[prop].duedate) {
                    ...
                    var juros_price_incorrer = sqlResults[prop].item == (ambiente == 'PRODUCTION' ? '' : 28654) ? sqlResults[prop].rate : ZERO;
                     // if (juros_price_incorrer > 0) {
                    //     log.audit(prop, juros_price_incorrer);
                    // }
                    ...
                    if (status == 'Aberto') {
                        arrayParcelas.push({
                            ...
                            // valorOriginal: vp || sqlResults[prop].foreigntotal,
                            valorOriginal: vp - juros_price_incorrer || sqlResults[prop].foreigntotal - juros_price_incorrer,
                            ...
                            // valorAtualizado: parcelaVencida.status == true ? valorAtualizado : sqlResults[prop].foreigntotal,                   
    ...
    if (method == 'GET') {
        ...
        for (i=0; i<arrayParcelas.length; i++) {
            ...
            sublista.setSublistValue({
                id: total.id,
                line: i,
                value: parseFloat(arrayParcelas[i].valorAtualizado) + parseFloat(installmentsSelected)
        ...
        for (i=0; i<arrayParcelas.length; i++) {
            ...
            sublista.setSublistValue({
                id: total.id,
                line: i,
                value: Number(arrayParcelas[i].total).toFixed(2)
            });
        }
    }
}
