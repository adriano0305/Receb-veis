 ...
 */
 function _applyCreditMemoCustomImpact(transactionRecord, standardLines, customLines) {   
  nlapiLogExecution('AUDIT', '_applyCreditMemoCustomImpact', JSON.stringify({
    customLines: customLines,
    standardLines: standardLines, 
    transactionRecord: transactionRecord
  }));
  ...
  for (i=0; i<standardLines.getCount(); i++) {
    var sl = standardLines.getLine(i);
    nlapiLogExecution('AUDIT', 'sl', JSON.stringify(sl));
    var amount = parseFloat(sl.getCreditAmount());

    if (sl.accountId != 109) {
      nlapiLogExecution('AUDIT', 'sl.getCreditAmount()', sl.getCreditAmount());
      nlapiLogExecution('AUDIT', 'sl.getDebitAmount()', sl.getDebitAmount());
      if (sl.getCreditAmount() > 0) {
        standardLinesList.push({
          accountId: sl.getAccountId(),
