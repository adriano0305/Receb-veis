...
const _get2 = (context) => {
    ...
    //        }),
    //        search.createColumn({
    //           name: "custbody_rsc_projeto_obra_gasto_compra",
    //           summary: "GROUP",
    ...
    //  log.audit("journalentrySearchObj result count",searchResultCount);

    //  var relacao = [];
    //  journalentrySearchObj.run().each(function(result){
    //     // .run().each has a limit of 4,000 results
    //     relacao.push(result.id);
    //     record.delete({type: 'journalentry', id: result.id});
    //     return true;
    //  });

    //  return relacao;

    record.delete({type: 'journalentry', id: 636862});
     
     /*
     journalentrySearchObj.id="customsearch1663625552066";
     journalentrySearchObj.title="BUSCA - Lançamentos (Geral) (copy)";
     var newSearchId = journalentrySearchObj.save();
     */
}
