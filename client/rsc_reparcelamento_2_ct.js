...
function simulacao(ordem) {
    ...
    for (i=0; i<registroAtual.getLineCount({sublistId: listaParcelas}); i++) {
        ...
        var indice = registroAtual.getSublistValue({sublistId: listaParcelas, fieldId: custPage+'indice', line: i});
        // Indice BRL: no cadastro desta unidade de correção não tem histórico de fator.
        if (indice == 1) {
            valorD2 = registroAtual.getSublistValue({sublistId: listaParcelas, fieldId: custPage+'valor_original', line: i});
        } else {
            var valorD2 = (
